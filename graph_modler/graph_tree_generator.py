...
class ObjectTree:
    ...
    def read(self, object_map, image_cord_map):

        try:
            assert type(object_map) == pd.DataFrame, f'object_map should be of type \
				{pd.DataFrame}. Received {type(object_map)}'
            assert type(image_cord_map) == np.ndarray, f'image_cord_map should be of type {np.ndarray} \
				. Received {type(image_cord_map)}'
            ...
            # check if image_cord_map is greyscale
            assert image_cord_map.ndim == 2, 'Check if the read image_cord_map is greyscale.'
    ...
    def connect(self, plot=False, export_df=False):
        ...
        # plot image_cord_map if plot==True
        if plot == True:
            ...
            # check if image_cord_map exists in folder
            try:
                if len(img) == None:
                    pass
            except:
            ...
            # plot if image_cord_map exists
                ...
                # write image_cord_map in same folder
                PLOT_PATH = \
                    './grapher_outputs/plots/' + \
                    'object_tree_' + str(self.count) + '.jpg'
        ...
        if export_df == True:
            ...
            CSV_PATH = \
                ...
                'connections_' + str(self.count) + '.csv'
...
class Graph:
    ...
    def _get_text_features(self, data):
        ...
        assert type(
            data) == str, f'Expected type {str}. Received {type(data)}.'
        ...
        features = []
        features.append([n_lower, n_upper, n_spaces,
                        n_alpha, n_numeric, n_digits])
